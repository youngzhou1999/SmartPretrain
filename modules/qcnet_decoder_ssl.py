...
class QCNetDecoder_ssl(nn.Module):
    """
    QCNetDecoder_ssl is a lightweight, self-supervised variant of the QCNet decoder module.

    Compared to the supervised version (QCNetDecoder), this version is optimized for 
    self-supervised learning tasks such as contrastive learning. Key differences include:

    - No output heads (MLPs) for trajectory regression.
    - No trajectory GRU embedding (traj_emb) or prediction refinement stage.
    - Outputs only intermediate agent representations (hidden states) over recurrent steps.
    - Simplified forward pass without final position, heading, scale, or uncertainty predictions.

    This design allows the model to learn rich, transferable representations of agent behavior 
    over time without requiring supervision in the form of ground-truth trajectories.
    """
    ...
    def __init__(self,
                 ...
                 head_dim: int,
                 dropout: float) -> None:
        super(QCNetDecoder_ssl, self).__init__()
