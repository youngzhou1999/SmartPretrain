...
class ArgoverseV2Dataset(Dataset):
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # for identify valid sub-scenarios in ssl
        predict_mask_ssl = torch.zeros(num_agents, self.num_steps, dtype=torch.bool)
        ...
        for track_id, track_df in df.groupby('track_id'):
            ...
            predict_mask_ssl[agent_idx, agent_steps] = True
            ...
            if not current_valid_mask[agent_idx]:
                predict_mask[agent_idx, self.num_historical_steps:] = False
                # key to rec loss
                predict_mask_ssl[agent_idx, self.num_historical_steps:] = False
        ...
        return {
            ...
            'predict_mask_ssl': predict_mask_ssl,
            'id': agent_id,
            'type': agent_type,
    ...
    def get(self, idx: int) -> HeteroData:
        if not self.is_pretrain:
            ...
            with open(self.processed_paths[idx], 'rb') as handle:
                data = pickle.load(handle)  # Load the pickled data sample

            # Set or clean keys to ensure compatibility across multiple datasets
            data['data_set'] = 1                # Tag this sample with its dataset source (here 1 for 'av2')
            data['agent'].pop('id')             # Remove agent ID (possibly unnecessary or inconsistent across datasets)

            # Remove 'height' info for non-Waymo data (may be Waymo-specific and not used in other maps)
            if data['city'] != 'waymo':
                data['map_polygon'].pop('height')

            # Convert to a PyTorch Geometric HeteroData object for model processing
            data = HeteroData(data)

            # Calculate the maximum valid start index for sampling a historical window
            max_start = self.num_future_steps - self.num_historical_steps

            # Randomly sample a start time for the first scene slice (used as the main view)
            start_t = random.randint(0, max_start)
            scene_t = self.get_scene_data(data.clone(), start=start_t, first=1)

            # Sample a second start time that is temporally offset from the first one,
            # ensuring the second view is not overlapping with the historical window of the first
            start_t_ = random.randint(start_t + self.num_historical_steps, self.num_future_steps)
            scene_t_ = self.get_scene_data(data.clone(), start=start_t_, first=0)

            # Record the start indices used for each view
            scene_t['start'] = start_t
            scene_t_['start'] = start_t_

            # Free memory from the original data
            del data

            # Pack the two scene views into a dictionary for return
            # 't' is the first view, 't_' is the second (typically for contrastive/temporal learning)
            data = {
                't_': scene_t_,  # Later or alternative time slice
                't': scene_t     # Initial time slice
            }
    ...
    def get_scene_data(self, data, start, first=1):
        """
        Extract a segment of scene data starting from 'start' frame index,
        and process masks depending on the dataset type and whether this is
        the first (anchor) or second (target) view.

        Args:
            data (dict): Input scene data (e.g., a HeteroData object or dict).
            start (int): Starting time frame index for slicing.
            first (int): Flag indicating if this is the first (1) or second (0) view.

        Returns:
            data (dict): Modified data dict with updated agent states and masks.
        """

        end = start + self.num_historical_steps  # Calculate end index of the segment

        # Backup full-length position and heading for later use (e.g., regression target)
        data['pos_110'] = data['agent']['position'].clone()
        data['heading_110'] = data['agent']['heading'].clone()

        # Slice time-series data to the [start:end] segment
        data['agent']['valid_mask'] = data['agent']['valid_mask'][:, start:end].clone()
        data['agent']['predict_mask_ssl'] = data['agent']['predict_mask_ssl'][:, start:end].clone()
        data['agent']['position'] = data['agent']['position'][:, start:end].clone()
        data['agent']['heading'] = data['agent']['heading'][:, start:end].clone()
        data['agent']['velocity'] = data['agent']['velocity'][:, start:end].clone()

        # ------------------------ Determine valid agents mask ------------------------ #
        if first:
            if data['data_set'] == 1:  # AV2
                data['data_mask'] = data['agent']['predict_mask_ssl'].sum(-1) == self.num_historical_steps

            elif data['data_set'] == 0:  # AV1
                idx_30 = 30 - start
                data['data_mask'] = data['agent']['predict_mask_ssl'][:, idx_30:].sum(-1) == 20 + start

            elif data['data_set'] == 2:  # Waymo
                idx_39 = 39 - data['waymo_start']
                idx_39 = idx_39 - start
                data['data_mask'] = data['agent']['predict_mask_ssl'][:, idx_39:].sum(-1) == data['waymo_start'] + 11 + start
                data.pop('waymo_start')
        else:
            data['data_mask'] = data['agent']['predict_mask_ssl'].sum(-1) == self.num_historical_steps

            if data['data_set'] == 0:
                valid_idx = start - 50
                data['data_mask'] = data['agent']['predict_mask_ssl'].sum(-1) == 30 - valid_idx

            if data['data_set'] == 2:
                data.pop('waymo_start')

        # ------------------------ Create index tensor for gather-based ops ------------------------ #
        seg_len = self.num_historical_steps
        num_ego = data['agent']['num_nodes']  # Number of ego agents

        seg_start = torch.tensor(start).long().repeat(num_ego)

        seg_start_2_end = seg_start.repeat(seg_len).reshape(seg_len, num_ego)
        for i in range(seg_len):
            seg_start_2_end[i] += i

        data['seg_start_2_end'] = seg_start_2_end.transpose(0, 1)  # shape: (num_ego, seg_len)
